...
const DEFAULT_AVATAR_URL = 'https://picsum.photos/seed/luxecar/100/100'; 
...
const UserDropdown: React.FC<UserDropdownProps> = ({ session, onLogout, onNavigate }) => {
    ...
    const userAvatar = session.user.user_metadata?.avatar_url || DEFAULT_AVATAR_URL;
    ...
    return (
        <div className="relative group/user" ref={dropdownRef}>
            ...
            >
                <img
                    src={userAvatar}
                    alt={`Profil de ${userEmail}`}
                    className="w-8 h-8 rounded-full object-cover border-2 border-gold-400 shadow-lg"
                />
